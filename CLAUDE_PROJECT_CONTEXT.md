...
|---|---|---|
| Word in progress | Space and `. , ! ? : ; ) ] }` commit the first candidate + the key, only if `CompletionPolicy` agrees. Enter never does. Backspace is always an ordinary delete (the post-completion undo was removed in 0.13.1 at the owner's request; Ctrl+Backspace covers it). | `SuggestionController.HandleBoundary/HandleBackspace`, `CompletionPolicy` |
| Between words | One Tab inserts prediction #1 (with a leading space if the caret is right after a word). Tab again within `PredictionCycleWindowMs` (1200; saved 900 migrates) replaces **exactly** what was inserted with #2, and so on; Shift+Tab goes back. Any typing ends the cycle, so Tab after typing inserts a fresh #1. Prediction #1 is always a single word (`ShapePredictions`); phrases over 10 chars are dropped. An echo guard (400 ms after our own edit) ignores the text service reporting our insertion back, which used to end the cycle in browsers. | `HandleTab`, the `Cycle` record |
| Tab on a word that is already complete and ranked first | 0.13.2: if the strip has any other word, Tab opens the cycle on slot #1 without changing the text, so Tab Tab reaches slot #2 (owner: typing "his" with "history" showing, the old next-word jump made "history" unreachable, and whether Tab Tab glided depended on whether the typed text happened to be a word). Only when #1 is the sole candidate does Tab predict the next word. Consequence: after a complete word, next-word prediction is Space then Tab, or Tab, pause (window expires), Tab. Regression check 7 now types a trailing space; 7b covers "his". | `HandleTab` |
...
| Visuals | Passive by default (no lens; when Space is armed, slot #1 gets the same selection highlight a Tab cycle uses — bold alone was invisible in some themes); active only during a Tab cycle. Fixed slot geometry: `SlotCount × PreferredSlotWidth`, no borrowing or growth — words shrink then middle-elide. Chips are a fixed pool updated in place. | `SuggestionBarWindow`, `SlotPanel`, `SuggestionChipViewModel` |
...
**0.13.1 additions (owner feedback on 0.13.0):**
- `EnglishForms` (Core/Prediction): written forms for the strip (`I'm`, `don't`, proper nouns), contraction candidates, and `SafeCorrections` applied on commit (`im`→`I'm`). Ambiguous pairs (`ill`, `were`, `well`, `its`, `lets`) and proper nouns that are ordinary words (`may`, `turkey`, `polish`) are deliberately excluded. Gated by `AppSettings.FixCapitalsAndApostrophes` (default on), independent of spelling `AutocorrectEnabled`.
- Sentence-start capitals use `TextContext.IsSentenceStartKnown` / `WordCommittedEventArgs.StartsSentence`: capitalise only when the start is *known* (after a typed terminator, or TSF says so), never on a guess.
- Browser path: the TIP only loads while it is in the user's keyboard list (HKCU, Control Panel > International > User Profile). `TipRegistrationManager.IsInKeyboardList/AddToKeyboardList` (InstallLayoutOrTip); Settings shows **Switch it on**, tray warns at startup. Never change the user's list without their click.
- 0.13.3: `SelectionLens.OnRender` must not skip drawing at `Opacity <= 0`. The lens is placed while transparent and faded in, and Opacity never re-runs OnRender, so the first pill (first Tab, or Space armed) drew nothing until a later Tab animated LensX. Found by screenshotting the bar; the armed state had only changed the text colour.
- `SuggestionBarWindow.WarmUp()` runs before hooks are installed: the first real render cost ~750 ms and dropped hook keystrokes.
...
| The backdrop measurement must be **quantised** before it reaches the separation floor | Raw samples differ every time, and each difference produced a slightly different surface — the material visibly drifted over a session. `BackdropTracker` settles it into bands; only a band change rebuilds the palette. |
| A dark variant is a dark *design*, not a pale one dimmed | Spatial Glass's pale veil and Editorial's inverted page both failed over dark applications. Author the dark variant on its own terms, and let a theme's two variants differ in design where the backdrop demands it (Command is grey by day and near-black at night). |
